...
#include "sframeutility.hpp" // sframe::ScopedCleanse
...
// Public in both 3.x and 4.x, but unlike the other Mbed TLS headers it carries no extern "C"
// guard of its own, so the C++ mangling has to be suppressed here or mbedtls_ct_memcmp does
// not link.
extern "C" {
...
#include "tls.hpp" // rtc::openssl::check
...
// The backends take unsigned char buffers. byte has the same size and object
...
size_t hash_length(gnutls_mac_algorithm_t hash) { return gnutls_hmac_get_len(hash); }
...
binary hmac(const binary &key, gnutls_mac_algorithm_t hash, const binary &data) {
	...
	if (ret < 0)
		throw std::runtime_error(string("HMAC failed: ") + gnutls_strerror(ret));
...
binary ctr_xcrypt(const binary &key, gnutls_cipher_algorithm_t cipher, const binary &nonce,
                  const binary &input) {
	...
	if (ret < 0)
		throw std::runtime_error(string("AES-CTR init failed: ") + gnutls_strerror(ret));

	unique_ptr<std::remove_pointer_t<gnutls_cipher_hd_t>, decltype(&gnutls_cipher_deinit)> ctx(
	    handle, gnutls_cipher_deinit);
	...
	while (offset < input.size()) {
		...
		if (ret < 0)
			throw std::runtime_error(string("AES-CTR encrypt failed: ") + gnutls_strerror(ret));
		...
		for (int i = 15; i >= 0; --i) {
			counter[i] = std::byte(std::to_integer<uint8_t>(counter[i]) + 1);
			if (counter[i] != std::byte{0})
				break; // no wrap, so nothing carries into the next byte
		}
	}
...
binary gcm_encrypt(const binary &key, gnutls_cipher_algorithm_t cipher, const binary &nonce,
                   const binary &aad, const binary &plaintext, size_t tagLength) {
	gnutls_datum_t keyDatum;
	keyDatum.data = const_cast<unsigned char *>(uc(key));
	keyDatum.size = (unsigned int)key.size();
	...
	if (ret < 0)
		throw std::runtime_error(string("AES-GCM init failed: ") + gnutls_strerror(ret));

	unique_ptr<std::remove_pointer_t<gnutls_aead_cipher_hd_t>, decltype(&gnutls_aead_cipher_deinit)>
	...
	ret = gnutls_aead_cipher_encrypt(ctx.get(), nonce.data(), nonce.size(), aad.data(), aad.size(),
	                                 tagLength, plaintext.data(), plaintext.size(), out.data(),
	                                 &outLen);
	if (ret < 0)
		throw std::runtime_error(string("AES-GCM encrypt failed: ") + gnutls_strerror(ret));

	out.resize(outLen);
	return out;
}

binary gcm_decrypt(const binary &key, gnutls_cipher_algorithm_t cipher, const binary &nonce,
                   const binary &aad, const binary &ciphertextAndTag, size_t tagLength) {
	...
	ret = gnutls_aead_cipher_decrypt(ctx.get(), nonce.data(), nonce.size(), aad.data(), aad.size(),
	                                 tagLength, ciphertextAndTag.data(), ciphertextAndTag.size(),
	                                 plaintext.data(), &plaintextLen);
	if (ret < 0) {
		if (ret == GNUTLS_E_DECRYPTION_FAILED)
			throw std::runtime_error("AES-GCM authentication failed");
		throw std::runtime_error(string("AES-GCM decrypt failed: ") + gnutls_strerror(ret));
	}
...
binary hkdf_extract(const binary &ikm, const binary &salt, gnutls_mac_algorithm_t hash) {
	...
	if (ret < 0)
		throw std::runtime_error(string("HKDF extract failed: ") + gnutls_strerror(ret));

	return prk;
}

binary hkdf_expand(const binary &prk, gnutls_mac_algorithm_t hash, const binary &info,
                   size_t length) {
	binary key(length);
	...
	if (ret < 0)
		throw std::runtime_error(string("HKDF expand failed: ") + gnutls_strerror(ret));

	return key;
}

bool constant_time_compare(const void *a, const void *b, size_t n) {
	return gnutls_memcmp(a, b, n) == 0;
}

void cleanse(void *p, size_t n) { gnutls_memset(p, 0, n); }
...
void cleanse(void *p, size_t n) { mbedtls_platform_zeroize(p, n); }

#if MBEDTLS_VERSION_MAJOR >= 4

// PSA has no strerror and mbedtls::check takes an int error code, so failures carry the numeric
// status rather than discarding it.
inline void check_psa(psa_status_t status, const char *message) {
	if (status != PSA_SUCCESS)
		throw std::runtime_error(string(message) + ", PSA status " + std::to_string(status));
}

namespace {

// sframe_crypto is reachable without ever creating a peer connection, so this backend
...
void ensure_init() {
	...
	// Routed through safe_psa() like every other PSA call in the codebase. On Mbed TLS 4 that is a
	// straight passthrough -- the build there refuses MBEDTLS_THREADING_C being off, so Mbed TLS
	// does its own locking -- but going through it keeps the one entry point for PSA calls.
	psa_status_t status =
	    static_cast<psa_status_t>(rtc::mbedtls::safe_psa([] { return psa_crypto_init(); }));
	if (status != PSA_SUCCESS)
		throw std::runtime_error("psa_crypto_init failed, PSA status " + std::to_string(status));
...
class Key final {
public:
	Key(psa_key_type_t type, psa_algorithm_t algorithm, psa_key_usage_t usage, const binary &key) {
		...
		if (status != PSA_SUCCESS)
			throw std::runtime_error("Failed to import key, PSA status " + std::to_string(status));
	}
...
void derivation_input(psa_key_derivation_operation_t *operation, psa_key_derivation_step_t step,
                      const binary &data) {
	...
	check_psa(psa_key_derivation_input_bytes(operation, step, ptr, data.size()),
	          "HKDF input failed");
}

} // namespace

size_t hash_length(psa_algorithm_t hash) {
	const size_t length = PSA_HASH_LENGTH(hash);
	if (length == 0)
		throw std::runtime_error("Unsupported hash type in sframe_crypto");
...
binary hmac(const binary &key, psa_algorithm_t hash, const binary &data) {
	...
	binary mac(hash_length(hash));
	size_t macLength = 0;
	check_psa(psa_mac_compute(macKey.id(), algorithm, uc(data), data.size(), uc(mac), mac.size(),
	                          &macLength),
	          "HMAC failed");
...
binary ctr_xcrypt(const binary &key, psa_algorithm_t cipher, const binary &nonce,
                  const binary &input) {
	binary output(input.size());
	if (input.empty())
		return output;
	...
	CipherOperation ctx;
	check_psa(psa_cipher_encrypt_setup(&ctx.operation, cipherKey.id(), cipher),
	          "AES-CTR setup failed");

	// AES-CTR requires a full 16-byte counter block; zero-pad the nonce if shorter.
	binary iv(nonce);
	iv.resize(16, std::byte{0});

	check_psa(psa_cipher_set_iv(&ctx.operation, uc(iv), iv.size()), "AES-CTR set iv failed");

	size_t outLen = 0, total = 0;
	check_psa(psa_cipher_update(&ctx.operation, uc(input), input.size(), uc(output), output.size(),
	                            &outLen),
	          "AES-CTR update failed");

	total = outLen;

	check_psa(psa_cipher_finish(&ctx.operation, uc(output) + total, output.size() - total, &outLen),
...
binary gcm_encrypt(const binary &key, psa_algorithm_t cipher, const binary &nonce,
                   const binary &aad, const binary &plaintext, size_t tagLength) {
	ensure_init();
	...
	binary out(plaintext.size() + tagLength);
	size_t outLen = 0;
	check_psa(psa_aead_encrypt(aeadKey.id(), algorithm, uc(nonce), nonce.size(), uc(aad),
	                           aad.size(), uc(plaintext), plaintext.size(), uc(out), out.size(),
	                           &outLen),
...
binary gcm_decrypt(const binary &key, psa_algorithm_t cipher, const binary &nonce,
                   const binary &aad, const binary &ciphertextAndTag, size_t tagLength) {
	if (ciphertextAndTag.size() < tagLength)
		throw std::invalid_argument("Payload shorter than tag length");
	...
	psa_status_t status = psa_aead_decrypt(
	    aeadKey.id(), algorithm, uc(nonce), nonce.size(), uc(aad), aad.size(), uc(ciphertextAndTag),
	    ciphertextAndTag.size(), uc(plaintext), plaintext.size(), &ptLen);
...
binary hkdf_extract(const binary &ikm, const binary &salt, psa_algorithm_t hash) {
	ensure_init();

	binary prk(hash_length(hash));

	DerivationOperation ctx;
	check_psa(psa_key_derivation_setup(&ctx.operation, PSA_ALG_HKDF_EXTRACT(hash)),
	          "HKDF extract setup failed");

	// HKDF-Extract takes the salt then the input keying material, in that order.
	derivation_input(&ctx.operation, PSA_KEY_DERIVATION_INPUT_SALT, salt);
	derivation_input(&ctx.operation, PSA_KEY_DERIVATION_INPUT_SECRET, ikm);

	check_psa(psa_key_derivation_output_bytes(&ctx.operation, uc(prk), prk.size()),
	          "HKDF extract failed");

	return prk;
}

binary hkdf_expand(const binary &prk, psa_algorithm_t hash, const binary &info, size_t length) {
	ensure_init();

	binary key(length);

	DerivationOperation ctx;
	check_psa(psa_key_derivation_setup(&ctx.operation, PSA_ALG_HKDF_EXPAND(hash)),
	          "HKDF expand setup failed");

	// HKDF-Expand takes the pseudorandom key then the info string, in that order.
	derivation_input(&ctx.operation, PSA_KEY_DERIVATION_INPUT_SECRET, prk);
	derivation_input(&ctx.operation, PSA_KEY_DERIVATION_INPUT_INFO, info);

	check_psa(psa_key_derivation_output_bytes(&ctx.operation, uc(key), key.size()),
	          "derive failed");
...
size_t hash_length(mbedtls_md_type_t hash) {
...
binary ctr_xcrypt(const binary &key, mbedtls_cipher_type_t cipher, const binary &nonce,
                  const binary &input) {
	binary output(input.size());
	...
	unique_ptr<mbedtls_cipher_context_t, void (*)(mbedtls_cipher_context_t *)> ctx(
...
binary gcm_encrypt(const binary &key, mbedtls_cipher_type_t cipher, const binary &nonce,
                   const binary &aad, const binary &plaintext, size_t tagLength) {
	(void)cipher; // Mbed TLS GCM is keyed by AES id + key length directly.

	unique_ptr<mbedtls_gcm_context, void (*)(mbedtls_gcm_context *)> ctx(
	...
	if (mbedtls_gcm_setkey(ctx.get(), MBEDTLS_CIPHER_ID_AES, uc(key),
	                       (unsigned int)(key.size() * 8)) != 0)
		throw std::runtime_error("AES-GCM set key failed");

	binary ct(plaintext.size());
	binary tag(tagLength);

	if (mbedtls_gcm_crypt_and_tag(ctx.get(), MBEDTLS_GCM_ENCRYPT, plaintext.size(), uc(nonce),
	                              nonce.size(), uc(aad), aad.size(), uc(plaintext), uc(ct),
	                              tag.size(), uc(tag)) != 0)
...
binary gcm_decrypt(const binary &key, mbedtls_cipher_type_t cipher, const binary &nonce,
                   const binary &aad, const binary &ciphertextAndTag, size_t tagLength) {
	(void)cipher; // Mbed TLS GCM is keyed by AES id + key length directly.
	...
	int ret = mbedtls_gcm_auth_decrypt(ctx.get(), ctLen, uc(nonce), nonce.size(), uc(aad),
	                                   aad.size(), tagPtr, tagLength, ctPtr, uc(plaintext));
...
binary hkdf_extract(const binary &ikm, const binary &salt, mbedtls_md_type_t hash) {
	const mbedtls_md_info_t *info = mbedtls_md_info_from_type(hash);
	if (!info)
...
binary hkdf_expand(const binary &prk, mbedtls_md_type_t hash, const binary &info, size_t length) {
	...
	if (mbedtls_hkdf_expand(mdInfo, uc(prk), prk.size(), uc(info), info.size(), uc(key), length) !=
	    0)
...
size_t hash_length(const EVP_MD *hash) { return EVP_MD_size(hash); }
...
binary ctr_xcrypt(const binary &key, const EVP_CIPHER *cipher, const binary &nonce,
                  const binary &input) {
	binary output(input.size());

	unique_ptr<EVP_CIPHER_CTX, decltype(&EVP_CIPHER_CTX_free)> ctx(EVP_CIPHER_CTX_new(),
	                                                               EVP_CIPHER_CTX_free);
	...
	check(EVP_EncryptInit_ex(ctx.get(), cipher, nullptr, uc(key), uc(iv)), "AES-CTR init failed");

	int len = 0, clen = 0;
	if (!input.empty() &&
	    (EVP_EncryptUpdate(ctx.get(), uc(output), &len, uc(input), (int)input.size()) != 1))
		throw std::runtime_error("AES-CTR update failed");

	clen = len;

	check(EVP_EncryptFinal_ex(ctx.get(), uc(output) + clen, &len), "AES-CTR final failed");
...
binary gcm_encrypt(const binary &key, const EVP_CIPHER *cipher, const binary &nonce,
                   const binary &aad, const binary &plaintext, size_t tagLength) {
	unique_ptr<EVP_CIPHER_CTX, decltype(&EVP_CIPHER_CTX_free)> ctx(EVP_CIPHER_CTX_new(),
	                                                               EVP_CIPHER_CTX_free);
	...
	check(EVP_EncryptInit_ex(ctx.get(), cipher, nullptr, uc(key), uc(nonce)),
	      "AES-GCM init failed");

	int len = 0;
	check(EVP_EncryptUpdate(ctx.get(), nullptr, &len, uc(aad), (int)aad.size()),
	      "AES-GCM AAD update failed");

	binary ct(plaintext.size());
	check(EVP_EncryptUpdate(ctx.get(), uc(ct), &len, uc(plaintext), (int)plaintext.size()),
	      "AES-GCM encrypt failed");

	int ciphertextLen = len;
	check(EVP_EncryptFinal_ex(ctx.get(), uc(ct) + len, &len), "AES-GCM final failed");

	ciphertextLen += len;
	ct.resize(ciphertextLen);

	binary tag(tagLength);
	check(EVP_CIPHER_CTX_ctrl(ctx.get(), EVP_CTRL_GCM_GET_TAG, (int)tag.size(), tag.data()),
...
binary gcm_decrypt(const binary &key, const EVP_CIPHER *cipher, const binary &nonce,
                   const binary &aad, const binary &ciphertextAndTag, size_t tagLength) {
	if (ciphertextAndTag.size() < tagLength)
		throw std::invalid_argument("Payload shorter than tag length");
	...
	check(EVP_DecryptInit_ex(ctx.get(), cipher, nullptr, uc(key), uc(nonce)),
	      "AES-GCM init failed");

	int len = 0;
	check(EVP_DecryptUpdate(ctx.get(), nullptr, &len, uc(aad), (int)aad.size()),
	      "AES-GCM AAD update failed");
	...
	if (EVP_CIPHER_CTX_ctrl(ctx.get(), EVP_CTRL_GCM_SET_TAG, (int)tagLength,
	                        const_cast<uint8_t *>(tagPtr)) != 1)
...
binary hkdf_extract(const binary &ikm, const binary &salt, const EVP_MD *hash) {
	const size_t hashLen = EVP_MD_size(hash);
	binary prk(hashLen);

	unique_ptr<EVP_PKEY_CTX, decltype(&EVP_PKEY_CTX_free)> pctx(
	    EVP_PKEY_CTX_new_id(EVP_PKEY_HKDF, NULL), EVP_PKEY_CTX_free);
...
binary hkdf_expand(const binary &prk, const EVP_MD *hash, const binary &info, size_t length) {
	binary key(length);
...
void cleanse(void *p, size_t n) { OPENSSL_cleanse(p, n); }
...
// The content key split into its encryption and authentication halves. Self-clearing and passed by
// reference everywhere below: this is built for every frame, so a plain copy would scatter the
// content key across freed heap at frame rate.
struct SFrameSubKeys {
	sframe::SecretBinary encryptionKey;
	sframe::SecretBinary authenticationKey;
};

static SFrameCipherSuiteInfo GetCipherSuiteInfo(uint16_t cipherSuiteId);
static SFrameSubKeys SplitSFrameKey(size_t combinedKeySize, size_t encryptionKeySize,
                                    const binary &sframeKey);
static binary ComputeTag(const SFrameSubKeys &keys, SFrameHashType hash, const binary &nonce,
                         const binary &aad, const binary &ct, size_t tagLength);
static bool VerifyTag(const SFrameSubKeys &keys, SFrameHashType hash, const binary &nonce,
                      const binary &aad, const binary &ciphertext, const binary &tag,
                      size_t tagLength);
static binary TruncateTag(const binary &tag, size_t n);
static binary EncodePlaintextForCTRHMAC(const SFrameSubKeys &keys,
                                        SFrameCipherSuiteInfo cipherSuiteInfo, const binary &nonce,
                                        const binary &aad, const binary &plaintext);
static binary DecodeCiphertextForCTRHMAC(const SFrameSubKeys &keys,
                                         SFrameCipherSuiteInfo cipherSuiteInfo, const binary &nonce,
                                         const binary &aad, const binary &ciphertextAndTag);
static binary derive_sframe_secret(const binary &baseKey, SFrameHashType suiteHash);
static binary derive_sframe_secret(const binary &baseKey, const binary &salt,
                                   SFrameHashType suiteHash);
static binary deriveSFrameKey(const binary &sframeSecret, SFrameHashType suiteHash,
                              const binary &sFrameKeyLabel, size_t keySize);
static binary deriveSFrameSalt(const binary &sframeSecret, SFrameHashType suiteHash,
                               const binary &sFrameSaltLabel, size_t nonceSize);
...
// which gnutls::ctr_xcrypt() synthesizes the CTR keystream.
#if USE_GNUTLS
...
// The SFrame cipher suites, as registered by IANA at https://www.iana.org/assignments/sframe
SFrameCipherSuiteInfo GetCipherSuiteInfo(uint16_t cipherSuiteId) {
	...
	default:
		throw std::runtime_error("Unsupported SFrame Cipher Suite ID in GetCipherSuiteInfo: " +
		                         std::to_string(cipherSuiteId));
	}
}
...
size_t sframe_crypto::GetNonceSize(uint16_t cipherSuiteId) {
	return GetCipherSuiteInfo(cipherSuiteId).nonceSize;
}

binary sframe_crypto::RatchetKey(uint16_t cipherSuiteId, const binary &baseKey) {
	...
	const size_t nh = backend::hash_length(suiteInfo.hkdfHash);

	// HKDF-Extract(salt=zeros/default, ikm=current_key)
	binary prk = derive_sframe_secret(baseKey, suiteInfo.hkdfHash);
	sframe::ScopedCleanse clearPrk(prk);

	// HKDF-Expand(prk, "SFrame 1.0 Ratchet", Nh)
	static const string kRatchetInfo = "SFrame 1.0 Ratchet";
	...
	return backend::hkdf_expand(prk, suiteInfo.hkdfHash, info, nh);
}

binary sframe_crypto::EncodePlaintext(uint16_t cipherSuiteId, const binary &sframeKey,
	...
	ValidateNonce(cipherSuiteInfo.nonceSize, nonce);

	SFrameSubKeys subKeys = SplitSFrameKey(cipherSuiteInfo.combinedKeySize,
	                                       cipherSuiteInfo.encryptionKeySize, sframeKey);
	...
	case 0x05:
		...
		return backend::gcm_encrypt(subKeys.encryptionKey.bytes(), cipherSuiteInfo.cipher, nonce,
		                            aad, plaintext, cipherSuiteInfo.tagLength);
	default:
		throw std::runtime_error(
		    "Unsupported SFrame Cipher Suite ID in sframe_crypto::EncodePlaintext: " +
		    std::to_string(cipherSuiteId));
	}
}

binary sframe_crypto::DecodeCiphertext(uint16_t cipherSuiteId, const binary &sframeKey,
                                       const binary &nonce, const binary &aad,
	...
	case 0x05:
		// For Suite 4-5, use AES-GCM mode
		return backend::gcm_decrypt(subKeys.encryptionKey.bytes(), cipherSuiteInfo.cipher, nonce,
		                            aad, ciphertextAndTag, cipherSuiteInfo.tagLength);
	default:
		throw std::runtime_error(
		    "Unsupported SFrame Cipher Suite ID in sframe_crypto::DecodeCiphertext: " +
		    std::to_string(cipherSuiteId));
	}
}

binary EncodePlaintextForCTRHMAC(const SFrameSubKeys &keys, SFrameCipherSuiteInfo cipherSuiteInfo,
                                 const binary &nonce, const binary &aad, const binary &plaintext) {
	auto ct =
	    backend::ctr_xcrypt(keys.encryptionKey.bytes(), cipherSuiteInfo.cipher, nonce, plaintext);

	auto tag =
	    ComputeTag(keys, cipherSuiteInfo.hkdfHash, nonce, aad, ct, cipherSuiteInfo.tagLength);
...
binary DecodeCiphertextForCTRHMAC(const SFrameSubKeys &keys, SFrameCipherSuiteInfo cipherSuiteInfo,
                                  const binary &nonce, const binary &aad,
                                  const binary &ciphertextAndTag) {
	...
	if (!VerifyTag(keys, cipherSuiteInfo.hkdfHash, nonce, aad, ciphertext, tag,
	               cipherSuiteInfo.tagLength)) {
		throw std::runtime_error("Authentication failed");
	}

	// AES-CTR decryption is just AES-CTR encryption
	auto pt =
	    backend::ctr_xcrypt(keys.encryptionKey.bytes(), cipherSuiteInfo.cipher, nonce, ciphertext);

	return pt;
}

binary ComputeTag(const SFrameSubKeys &keys, SFrameHashType hash, const binary &nonce,
                  const binary &aad, const binary &ct, size_t tagLength) {
	...
	binary fullTag = backend::hmac(keys.authenticationKey.bytes(), hash, authData);
	return TruncateTag(fullTag, tagLength);
}

bool VerifyTag(const SFrameSubKeys &keys, SFrameHashType hash, const binary &nonce,
               const binary &aad, const binary &ciphertext, const binary &tag, size_t tagLength) {
	...
	return expected.size() == tag.size() &&
	       backend::constant_time_compare(uc(expected), uc(tag), tag.size());
}
...
SFrameSubKeys SplitSFrameKey(size_t combinedKeySize, size_t encryptionKeySize,
                             const binary &sframeKey) {
	...
	if (encryptionKeySize > combinedKeySize)
		throw std::invalid_argument("Cipher suite encryption key is longer than its combined key");

	if (encryptionKeySize == combinedKeySize)
		return {sframeKey, {}};

	return {binary(sframeKey.begin(), sframeKey.begin() + encryptionKeySize),
	        binary(sframeKey.begin() + encryptionKeySize, sframeKey.end())};
}

bool sframe_crypto::ConstantTimeEquals(const binary &a, const binary &b) {
	if (a.size() != b.size())
		return false;

	return backend::constant_time_compare(uc(a), uc(b), a.size());
}

void sframe_crypto::Cleanse(binary &buffer) {
	// An empty vector's data() may be null, and the backends take a plain pointer.
	if (buffer.empty())
		return;

	backend::cleanse(buffer.data(), buffer.size());
}

void sframe_crypto::ValidateCipherSuite(uint16_t cipherSuiteId) {
	// invalid_argument, not the runtime_error GetCipherSuiteInfo raises: this is the API boundary,
	// where an unregistered suite is a caller mistake, and the providers document one type for all
	// their rejections.
	try {
		GetCipherSuiteInfo(cipherSuiteId);
	} catch (const std::runtime_error &e) {
		throw std::invalid_argument(e.what());
	}
}

void sframe_crypto::ValidateBaseKey(const binary &baseKey) {
...
// The suite fixes the nonce length, and the backends do not agree on what a wrong one does: OpenSSL
// reads its cipher's own IV length regardless, so a short nonce is an over-read there while the
// other three return an error. Refused here instead, at the one boundary both exported entry points
// cross.
void sframe_crypto::ValidateNonce(size_t nonceSize, const binary &nonce) {
	if (nonce.size() != nonceSize)
		throw std::invalid_argument("SFrame nonce must be " + std::to_string(nonceSize) +
		                            " bytes for this cipher suite, got " +
		                            std::to_string(nonce.size()));
}

// Derive the material needed to encode sframes using various HKDF functions
SFrameDerivedKeys sframe_crypto::DeriveSFrameKeys(const binary &baseKey, uint16_t cipherSuiteId,
                                                  const binary &keyLabel,
                                                  const binary &saltLabel) {
	...
	binary sframeSecret = derive_sframe_secret(baseKey, cipherSuiteInfo.hkdfHash);

	// The HKDF secret is as good as the key: everything below derives from it.
	sframe::ScopedCleanse clearSecret(sframeSecret);

	binary sframeKey = deriveSFrameKey(sframeSecret, cipherSuiteInfo.hkdfHash, keyLabel,
	                                   cipherSuiteInfo.combinedKeySize);

	binary sframeSalt = deriveSFrameSalt(sframeSecret, cipherSuiteInfo.hkdfHash, saltLabel,
	                                     cipherSuiteInfo.nonceSize);

	// Moved rather than copied, so there is one set of these bytes to account for rather than two.
	return SFrameDerivedKeys{std::move(sframeKey), std::move(sframeSalt)};
}
...
binary sframe_crypto::DeriveSSRCKey(SSRC ssrc, const binary &baseKey, uint16_t cipherSuiteId) {
	ValidateBaseKey(baseKey);
	...
	binary prk = derive_sframe_secret(baseKey, ssrcSalt, suiteInfo.hkdfHash);
	sframe::ScopedCleanse clearPrk(prk);

	// HKDF-Expand(prk, info="SFrame 1.0 RTP Stream", L=Nh)
	static const string kInfo = "SFrame 1.0 RTP Stream";
...
binary derive_sframe_secret(const binary &baseKey, SFrameHashType suiteHash) {
	// RFC 5869 Section 2.2: an absent salt is a hash-length string of zeros, so the salted form
	// covers both cases and each backend needs only one extract.
	return derive_sframe_secret(baseKey, binary(backend::hash_length(suiteHash), std::byte{0}),
	                            suiteHash);
}

// Derive sframe secret using HKDF-Extract with an explicit salt value.
binary derive_sframe_secret(const binary &baseKey, const binary &salt, SFrameHashType suiteHash) {
	return backend::hkdf_extract(baseKey, salt, suiteHash);
}

// Derive sframe key using HKDF-Expand - RFC 9605 Section 4.4.2.
binary deriveSFrameKey(const binary &sframeSecret, SFrameHashType suiteHash,
                       const binary &sFrameKeyLabel, size_t keySize) {
	return backend::hkdf_expand(sframeSecret, suiteHash, sFrameKeyLabel, keySize);
}

// Derive sframe salt using HKDF-Expand - RFC 9605 Section 4.4.2.
binary deriveSFrameSalt(const binary &sframeSecret, SFrameHashType suiteHash,
                        const binary &sFrameSaltLabel, size_t nonceSize) {
	return backend::hkdf_expand(sframeSecret, suiteHash, sFrameSaltLabel, nonceSize);
}
