...
void PliHandler::incoming(message_vector &messages, [[maybe_unused]] const message_callback &send) {
	for (const auto &message : messages) {
		...
		while ((sizeof(RtcpHeader) + offset) <= message->size()) {
			...
			// Ensure the full RTCP packet is within bounds before reading it
			...
			if (payload_type == 196) {
				...
				if (feedback_message_type == 1) {
					mOnPli();
					break;
				} else if (feedback_message_type == 4 && length >= sizeof(RtcpFir)) {
					auto fir = reinterpret_cast<const RtcpFir *>(message->data() + offset);
					bool invokeHandler = false;
					int fciCount = fir->getFciCount();
					for (int i = 0; i < fciCount; i++) {
						const RtcpFirFci *fci = fir->getFci(i);
						if (!fci)
							break;
						uint8_t firSeqNo = fci->getSeqNo();
						...
						uint64_t comboSSRCs = (uint64_t(fir->header.packetSenderSSRC()) << 32) + fci->getSSRC();
