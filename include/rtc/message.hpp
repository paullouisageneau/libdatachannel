...
[[nodiscard]] inline size_t message_size_func(const message_ptr &m) {
	return m->type == Message::Binary || m->type == Message::String ? m->size() : 0;
}

template <typename Iterator>
[[nodiscard]] message_ptr make_message(Iterator begin, Iterator end, Message::Type type = Message::Binary,
...
template <typename Iterator>
[[nodiscard]] message_ptr make_message(Iterator begin, Iterator end, shared_ptr<FrameInfo> frameInfo) {
...
RTC_CPP_EXPORT [[nodiscard]] message_ptr make_message(size_t size, Message::Type type = Message::Binary,
                                        unsigned int stream = 0,
                                        shared_ptr<Reliability> reliability = nullptr);

RTC_CPP_EXPORT [[nodiscard]] message_ptr make_message(binary &&data, Message::Type type = Message::Binary,
                                        unsigned int stream = 0,
                                        shared_ptr<Reliability> reliability = nullptr);

RTC_CPP_EXPORT [[nodiscard]] message_ptr make_message(binary &&data, shared_ptr<FrameInfo> frameInfo);

RTC_CPP_EXPORT [[nodiscard]] message_ptr make_message(size_t size, message_ptr orig);

RTC_CPP_EXPORT [[nodiscard]] message_ptr make_message(message_variant data);

#if RTC_ENABLE_MEDIA

// Reconstructs a message_ptr from an opaque rtcMessage pointer that
// was allocated by rtcCreateOpaqueMessage().
[[nodiscard]] message_ptr make_message_from_opaque_ptr(rtcMessage *&&message);
